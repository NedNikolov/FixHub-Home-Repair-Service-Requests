import{r as p}from"./navbar-CFLUKmNY.js";const b=[{icon:"bi-water",title:"Emergency repairs",description:"Fast responses for leaks, electrical issues, broken fixtures, and urgent home problems."},{icon:"bi-tools",title:"Routine maintenance",description:"Scheduled upkeep for small fixes, seasonal checks, and preventative service requests."},{icon:"bi-house-heart",title:"Home improvement",description:"Simple upgrades and refresh work to keep your space functional, safe, and comfortable."}],h=[{icon:"bi-person-check",title:"Trusted Professionals",description:"Connect with experienced repair specialists who bring dependable service to every job."},{icon:"bi-lightning-charge-fill",title:"Fast Response",description:"Move from request to action quickly with a platform designed for urgent home issues."},{icon:"bi-shield-lock-fill",title:"Secure Platform",description:"Keep requests and customer details protected with a clean, reliable digital flow."},{icon:"bi-camera-fill",title:"Photo Uploads",description:"Share visual details with your request so repairs can be diagnosed more accurately."}],v=[{icon:"bi-lightning-charge",title:"Electrical"},{icon:"bi-droplet-fill",title:"Plumbing"},{icon:"bi-brush-fill",title:"Painting"},{icon:"bi-house-gear-fill",title:"Renovation"},{icon:"bi-snow2",title:"HVAC"},{icon:"bi-window",title:"Windows & Doors"}],s=[{icon:"bi-person-plus-fill",title:"Create Account"},{icon:"bi-journal-plus",title:"Submit Request"},{icon:"bi-cloud-upload-fill",title:"Upload Photos"},{icon:"bi-person-badge-fill",title:"Technician Review"},{icon:"bi-check2-circle",title:"Repair Completed"}],u=[{label:"Home",href:"#home"},{label:"Services",href:"#services"},{label:"About",href:"#about"},{label:"Contact",href:"#contact"}],m=[{label:"Privacy Policy",href:"#privacy-policy"},{label:"Terms of Service",href:"#terms-of-service"}],f=[{label:"GitHub",href:"https://github.com",icon:"bi-github"},{label:"Facebook",href:"https://facebook.com",icon:"bi-facebook"},{label:"LinkedIn",href:"https://linkedin.com",icon:"bi-linkedin"}];function g(){const l=b.map(e=>`
        <div class="col-md-4">
          <article class="feature-card h-100">
            <div class="service-pill mb-3"><i class="bi ${e.icon}"></i> Service</div>
      ...
      `).join(""),t=h.map(e=>`
        <div class="col-md-6 col-xl-3">
          <article class="feature-card h-100 hover-lift">
            <div class="card-icon mb-3"><i class="bi ${e.icon}"></i></div>
            <h2 class="h4 fw-bold">${e.title}</h2>
            <p class="text-secondary mb-0">${e.description}</p>
          </article>
        </div>
        ...
        <div class="col-sm-6 col-lg-4">
          <article class="feature-card service-card h-100 hover-lift">
            <div class="service-card-top">
              <div class="card-icon mb-3"><i class="bi ${e.icon}"></i></div>
              <h3 class="h4 fw-bold mb-0">${e.title}</h3>
            </div>
      ...
      `).join(""),c=s.map((e,i)=>`
        <div class="col-12 col-lg step-col">
          <article class="feature-card step-card h-100 hover-lift">
            <div class="step-badge mb-3">Step ${i+1}</div>
            <div class="card-icon mb-3"><i class="bi ${e.icon}"></i></div>
            <h3 class="h4 fw-bold mb-0">${e.title}</h3>
          </article>
          ${i<s.length-1?'<div class="step-connector d-none d-lg-flex"><i class="bi bi-arrow-right-short"></i></div>':""}
        </div>
      `).join(""),r=u.map(e=>`
        <li><a class="footer-link" href="${e.href}">${e.label}</a></li>
      `).join(""),n=m.map(e=>`
        <li><a class="footer-link" href="${e.href}">${e.label}</a></li>
      `).join(""),d=f.map(e=>`
        <a class="social-link" href="${e.href}" aria-label="${e.label}" target="_blank" rel="noreferrer">
          <i class="bi ${e.icon}"></i>
        </a>
      `).join("");return`
  ...
  `}const a=document.querySelector("#app");a&&(a.innerHTML=`
    ${p()}
    <main>
      ${g()}
    </main>
